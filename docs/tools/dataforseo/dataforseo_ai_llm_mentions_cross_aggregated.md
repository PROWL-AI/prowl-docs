---
name: dataforseo_ai_llm_mentions_cross_aggregated
provider: DataForSEO
...
Get cross-aggregated LLM mention metrics — compare mentions across multiple AI platforms.
...
```json
{
  ...
  "tool_name": "dataforseo_ai_llm_mentions_cross_aggregated",
  "params": {
    "targets": []
  }
}
...
|-------|------|----------|---------|-------------|
| `targets` | string[] | yes |  | List of 2-10 domains or keywords to compare |
...
| `tasks[].data.targets[]` | array<object> |  |
| `tasks[].data.targets[].aggregation_key` | string |  |
| `tasks[].data.targets[].target[]` | array<object> |  |
| `tasks[].data.targets[].target[].domain` | string |  |
...
```json
{
  ...
  "tasks": [
    {
      "id": "08111840-1544-0636-0000-fa1a390013cb",
      "status_code": 20000,
      "status_message": "Ok.",
      "time": "5.5111 sec.",
      "cost": 0.101,
...
- Cross-platform aggregated mention metrics. Compare brand visibility across multiple AI platforms simultaneously.
- Input: target, optional location_name, language_name.
- Returns per-platform breakdown — identify which AI platforms mention the brand most.
