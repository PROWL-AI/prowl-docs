---
name: dataforseo_bl_bulk_backlinks
provider: DataForSEO
...
Get total backlink counts for up to 1000 domains/pages in one request.
...
```json
{
  ...
  "tool_name": "dataforseo_bl_bulk_backlinks",
  "params": {
    "targets": []
  }
}
...
| `tasks[].result[].items[].backlinks` | integer |  |
...
```json
{
  ...
  "tasks": [
    {
      "id": "08111839-1544-0348-0000-a4afe0a4d964",
      "status_code": 20000,
      "status_message": "Ok.",
      "time": "0.0064 sec.",
      "cost": 0.024072,
...
- Total backlink counts for up to 1000 domains/pages in one request.
- Input: targets (list of up to 1000 URLs/domains).
- Returns per-target: total backlinks, dofollow, nofollow counts.
