---
name: dataforseo_bl_bulk_new_lost_referring_domains
provider: DataForSEO
...
Get new and lost referring domain counts for up to 1000 domains — track domain link growth.
...
```json
{
  ...
  "tool_name": "dataforseo_bl_bulk_new_lost_referring_domains",
  "params": {
    "targets": []
  }
}
...
| `tasks[].result[].items[].new_referring_domains` | integer |  |
| `tasks[].result[].items[].lost_referring_domains` | integer |  |
...
```json
{
  ...
  "tasks": [
    {
      "id": "08111839-1544-0350-0000-052e950ea6a6",
      "status_code": 20000,
      "status_message": "Ok.",
      "time": "0.1314 sec.",
      "cost": 0.024072,
...
- New and lost referring domain counts for up to 1000 domains.
- Input: targets, optional date_from/date_to.
- Returns new_referring_domains and lost_referring_domains per target.
