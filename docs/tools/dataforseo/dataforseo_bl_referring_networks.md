---
name: dataforseo_bl_referring_networks
provider: DataForSEO
...
# `dataforseo_bl_referring_networks`

Get referring IP networks — overview of referring IPs and subnets pointing to the target.
...
```json
{
  ...
  "tool_name": "dataforseo_bl_referring_networks",
  "params": {
    "target": "example.com"
  }
}
...
| `network_address_type` | enum(ip, subnet) | no |  | indicates the type of network to get data for |
...
```json
{
  ...
  "tasks": [
    {
      "id": "08111839-1544-0332-0000-e57108a26cee",
      "status_code": 20000,
      "status_message": "Ok.",
      "time": "0.0913 sec.",
      "cost": 0.0276,
...
- IP network analysis of backlinks. Returns referring IP addresses and subnets with backlink counts.
- Input: target, optional limit, filters, order_by.
- Useful for detecting PBN (private blog network) patterns — many links from same IP subnet = suspicious.
