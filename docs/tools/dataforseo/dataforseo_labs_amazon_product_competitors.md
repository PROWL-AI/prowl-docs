---
name: dataforseo_labs_amazon_product_competitors
provider: DataForSEO
...
Identify competing Amazon products by keyword overlap with a target ASIN.
...
| Tags | `amazon`, `competitors`, `dataforseo` |
...
```json
{
  ...
  "tool_name": "dataforseo_labs_amazon_product_competitors",
  "params": {
    "asin": "B08N5WRWNW"
  }
}
...
|-------|------|----------|---------|-------------|
| `asin` | string | yes |  | Target Amazon ASIN to find competitors for. |
...
| `limit` | integer | no |  | Max competing products returned (default 100, max 1000). |
...
```json
{
  ...
  "properties": {
    "asin": {
      "type": "string",
      "description": "Target Amazon ASIN to find competitors for."
    },
    "location_name": {
      "type": "string",
    ...
    "limit": {
      "type": "integer",
      "description": "Max competing products returned (default 100, max 1000)."
    },
    "offset": {
      "type": "integer",
  ...
  "description": "Parameters for the DataForSEO `dataforseo_labs/amazon/product_competitors/live` live endpoint. Listed properties are the curated core; additional keys are accepted and forwarded verbatim to the endpoint (full field list: https://docs.dataforseo.com/v3/).",
  "required": [
    "asin"
  ]
}
