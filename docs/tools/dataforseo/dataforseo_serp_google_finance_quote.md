---
name: dataforseo_serp_google_finance_quote
provider: DataForSEO
...
Get Google Finance quote data — stock price, market cap, P/E, and financial metrics for a ticker.
...
```json
{
  ...
  "tool_name": "dataforseo_serp_google_finance_quote",
  "params": {
...
| `window` | enum(1D, 5D, 1M, 6M, YTD, 1Y, 5Y, MAX) | no |  | time window for google_finance_quote graph |
| `keyword` | string | yes |  | Stock ticker symbol (e.g. 'AAPL', 'GOOG') |
...
```json
{
  ...
  "properties": {
    ...
    "window": {
      "type": "string",
      "description": "time window for google_finance_quote graph",
      "enum": [
        "1D",
        "5D",
        "1M",
...
```json
{
  ...
  "tasks": [
    {
      "id": "08111838-1544-0139-0000-89881ddfee03",
      "status_code": 40102,
