---
name: dataforseo_serp_google_finance_ticker_search
provider: DataForSEO
...
Search Google Finance for a ticker symbol — find matching stocks, funds, and ETFs.
...
| Tags | `dataforseo`, `finance`, `google`, `search`, `serp` |
...
```json
{
  ...
  "tool_name": "dataforseo_serp_google_finance_ticker_search",
  "params": {
...
|-------|------|----------|---------|-------------|
| `category` | enum(all, stock, index, mutual_fund, currency, futures) | no |  | category of financial instruments to search for |
...
```json
{
  ...
  "properties": {
    "category": {
      "type": "string",
      "description": "category of financial instruments to search for",
      "enum": [
        "all",
        "stock",
        "index",
...
```json
{
  ...
  "tasks": [
    {
      "id": "08111838-1544-0139-0000-d25ffa5aa1d8",
      "status_code": 20000,
      "status_message": "Ok.",
      "time": "12.4773 sec.",
      "cost": 0.002,
...
- Search for stock tickers by company name — prerequisite for finance_quote when ticker is unknown.
