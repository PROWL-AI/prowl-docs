---
name: keywords_everywhere_currencies
...
# `keywords_everywhere_currencies`

Supported currency codes for CPC values returned by keywords_everywhere_keyword_data.
...
```json
{
  ...
  "tool_name": "keywords_everywhere_currencies",
  "params": {}
}
```
...
Currency-code → currency-name map plus a count

Key fields: `currencies`, `count`
...
- CPC is reported in the requested currency — keep one currency across a report for comparability
