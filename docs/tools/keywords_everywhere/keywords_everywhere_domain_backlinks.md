---
name: keywords_everywhere_domain_backlinks
...
# `keywords_everywhere_domain_backlinks`

Top backlinks pointing at a domain: source URL/domain, target URL/domain, anchor text.
...
```json
{
  ...
  "tool_name": "keywords_everywhere_domain_backlinks",
  "params": {
    "domain": "example.com"
  }
}
...
data[] backlink rows + credit accounting

Key fields: `data[].anchor_text`, `data[].domain_source`, `data[].url_source`, `data[].domain_target`, `data[].url_target`, `credits_consumed`
...
- Raw rows can be dominated by one loud referrer — switch to the unique_* variant for a referring-domain view
- No trust/authority score here: cross-check quality with majestic_get_index_item_info (TrustFlow) or dataforseo_bl_summary
