---
name: keywords_everywhere_pasf_keywords
...
# `keywords_everywhere_pasf_keywords`

'People Also Search For' keywords for one seed keyword — Google's post-click refinement queries, useful for intent mapping and answer-engine coverage.
...
```json
{
  ...
  "tool_name": "keywords_everywhere_pasf_keywords",
  "params": {
...
data[] of PASF keyword strings + credit accounting
...
- PASF reflects post-click behaviour — pairs well with dataforseo_labs_related_keywords for query-intent clustering
- Returns strings only — price them with keywords_everywhere_keyword_data
