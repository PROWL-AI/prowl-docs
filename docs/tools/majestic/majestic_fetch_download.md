...
# `majestic_fetch_download`

Fetch and parse a FINISHED Majestic download file (gzip CSV) from its PublicDownloadLocation (https://downloads.majestic.com/...
...
```json
{
  ...
  "tool_name": "majestic_fetch_download",
  "params": {
    "url": "https://downloads.majestic.com/example-report.gz"
  }
}
```
...
|-------|------|----------|---------|-------------|
| `url` | string | yes |  | PublicDownloadLocation from majestic_get_downloads_list (StatusLabel=ready_for_download) |
| `max_rows` | integer | no | `1000` |  |
...
```json
{
  ...
  "properties": {
    "url": {
      "type": "string",
      "description": "PublicDownloadLocation from majestic_get_downloads_list (StatusLabel=ready_for_download)"
    },
    "max_rows": {
      "type": "integer",
      "default": 1000,
...
Key fields: `headers`, `rows`, `row_count`, `truncated`
...
- truncated=true means the file has more rows than max_rows — the full file stays available until Expires

- Chain dependency: obtain a live `PublicDownloadLocation` URL from `majestic_get_downloads_list` (job must reach StatusLabel=ready_for_download), then pass it here.

**Chain inputs:** `{'param': 'url', 'from_tool': 'majestic_get_downloads_list', 'extract': 'DataTables.Downloads.Data[].PublicDownloadLocation'}`
