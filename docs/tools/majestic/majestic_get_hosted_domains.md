---
name: majestic_get_hosted_domains
provider: Majestic
...
# `majestic_get_hosted_domains`

Domains co-hosted on the same IP / subnet as a domain or IPv4 — bad-neighborhood and PBN detection.
...
```json
{
  ...
  "tool_name": "majestic_get_hosted_domains",
  "params": {
    "domain": "majestic.com"
  }
}
...
```json
{
  ...
  "properties": {
    "domain": {
      "type": "string",
      "description": "Root domain or IPv4 address"
    },
    "datasource": {
      "type": "string",
      "enum": [
...
DataTables.DomainsOnIP + DataTables.DomainsOnSubnet; envelope has RecommendedIP

Key fields: `DataTables.DomainsOnIP.Data[].Domain`, `DataTables.DomainsOnSubnet.Data[].TrustFlow`
...
- Empty DomainsOnIP with round-robin DNS? Re-query using the envelope's RecommendedIP
