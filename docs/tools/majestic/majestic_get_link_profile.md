---
name: majestic_get_link_profile
provider: Majestic
...
# `majestic_get_link_profile`

CF/TF scatter-matrix data (link-profile graph) for up to 5 items — the distribution of backlink quality, ideal for PCM visualizations comparing link profiles.
...
```json
{
  ...
  "tool_name": "majestic_get_link_profile",
  "params": {
    "items": [
      "majestic.com",
      "ahrefs.com"
...
| `datatypes` | integer[] | no |  | Per-item: 0=link-level profile; 1=referring domains (must match items length) |
...
```json
{
  ...
  "properties": {
    ...
    "datatypes": {
      ...
      "description": "Per-item: 0=link-level profile; 1=referring domains (must match items length)"
    },
    "use_prefix_scan": {
      "type": "boolean",
      "default": false
...
DataTables.Items + dynamic RankingMatrix_N tables (CitationFlow × TrustFlow × Value)

Key fields: `DataTables.Items.Data[].MatrixTableName`
...
- Resolve each item's matrix table via Items.MatrixTableName — table names are dynamic
