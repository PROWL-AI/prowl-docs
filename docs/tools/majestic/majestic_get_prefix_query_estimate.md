---
name: majestic_get_prefix_query_estimate
provider: Majestic
...
# `majestic_get_prefix_query_estimate`

Feasibility pre-check for use_prefix_scan=True on other majestic tools: whether a real-time path-prefix query (example.com/blog*) is possible, with estimated backlink counts.
...
```json
{
  ...
  "tool_name": "majestic_get_prefix_query_estimate",
  "params": {
    "item": "majestic.com/blog"
  }
}
```
...
|-------|------|----------|---------|-------------|
| `item` | string | yes |  | URL prefix, e.g. example.com/blog |
| `exact_mode` | boolean | no | `False` | Exact count check when the estimate is low |
...
```json
{
  ...
  "properties": {
    "item": {
      "type": "string",
      "description": "URL prefix, e.g. example.com/blog"
    },
    "exact_mode": {
      "type": "boolean",
      "default": false,
      "description": "Exact count check when the estimate is low"
    }
  },
  "required": [
    "item"
...
Envelope-only fields (no DataTables)

Key fields: `RealTimeQueryAllowed`, `EstimatedExtBacklinks`, `ActualExtBackLinks`
...
- RealTimeQueryAllowed=false → prefix scans will fail; fall back to majestic_download_back_links
