---
name: majestic_get_ref_domain_info
provider: Majestic
...
# `majestic_get_ref_domain_info`

Batch domain-level metrics for up to 999 root domains: TrustFlow, CitationFlow, ExtBackLinks, RefDomains, geo (country/city/lat-long), TLD, topical Trust Flow.
...
```json
{
  ...
  "tool_name": "majestic_get_ref_domain_info",
  "params": {
    "items": [
      "majestic.com",
      "ahrefs.com"
...
|-------|------|----------|---------|-------------|
| `items` | string[] | yes |  | Root domains (hostnames are auto-reduced to root) |
...
```json
{
  ...
  "properties": {
    "items": {
      ...
      "minItems": 1,
      "maxItems": 999,
      "description": "Root domains (hostnames are auto-reduced to root)"
    },
    "datasource": {
      "type": "string",
      "enum": [
...
DataTables.DomainsInfo rows (one per domain)

Key fields: `DataTables.DomainsInfo.Data[].TrustFlow`, `DataTables.DomainsInfo.Data[].CitationFlow`, `DataTables.DomainsInfo.Data[].RefDomains`, `DataTables.DomainsInfo.Data[].Status`
...
- Uncrawled domains return Status=NotFound
- Fixed cost per call — prefer one 500-domain call over 10 small ones
