---
name: majestic_get_related_sites
provider: Majestic
...
# `majestic_get_related_sites`

Co-citation analysis (Beta): domains mentioned around backlinks to the item — 'sites like X' from the link graph, with TrustFlow and co-occurrence counts.
...
```json
{
  ...
  "tool_name": "majestic_get_related_sites",
  "params": {
    "item": "majestic.com"
  }
}
...
DataTables.Domains rows (≤100)

Key fields: `DataTables.Domains.Data[].Domain`, `DataTables.Domains.Data[].Instances`, `DataTables.Domains.Data[].TrustFlow`
...
- Beta endpoint — treat as competitor-discovery signal, cross-check with dataforseo_labs_competitors_domain
