---
name: majestic_submit_urls_to_crawl
provider: Majestic
...
# `majestic_submit_urls_to_crawl`

Submit up to 100 URLs/domains to Majestic's crawler (best-effort, no completion signal).
...
```json
{
  ...
  "tool_name": "majestic_submit_urls_to_crawl",
  "params": {
    "items": [
      "example.com"
    ]
...
- Fire-and-forget: there is no crawl-status polling; re-check the index after ~days
