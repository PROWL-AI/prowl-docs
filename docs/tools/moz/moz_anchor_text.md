...
Anchor-text profile for a target — which phrases the web uses to link to it, with per-anchor link and domain counts.
...
Key fields: `anchor_texts`, `offset`
...
- Compare the branded / exact-match / generic anchor mix against majestic_get_anchor_text
