...
Moz Priority score (1-100) for a keyword — volume, difficulty, and organic CTR blended into one ranking of where to spend effort first.
...
```json
{
  ...
  "tool_name": "moz_keyword_priority",
  "params": {
    "keyword": "seo tools"
  }
}
...
keyword_metrics with priority

Key fields: `keyword_metrics.priority`
...
- Priority is Moz's own weighting — state it as Moz's opinion, not an objective ranking
