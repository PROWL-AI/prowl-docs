...
Inbound links pointing at a target from the Moz link index — a third independent backlink index next to Majestic and DataForSEO.
...
links[] rows plus offset/options

Key fields: `links`, `offset`, `options`
...
- Every returned row bills quota — start at limit=25 and widen only when the sample is too thin
