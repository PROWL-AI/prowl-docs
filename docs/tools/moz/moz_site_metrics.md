...
Moz flagship: full metric set for one domain, subdomain, or URL — Domain Authority, Page Authority, Spam Score, linking root domains, and total inbound links.
...
|-------|------|----------|---------|-------------|
| `query` | string | yes |  | Domain, subdomain, or URL (e.g. moz.com) |
...
```json
{
  ...
  "properties": {
    "query": {
      "type": "string",
      "description": "Domain, subdomain, or URL (e.g. moz.com)"
    },
    "scope": {
      "type": "string",
      "enum": [
...
site_metrics object for the queried site

Key fields: `site_metrics.domain_authority`, `site_metrics.page_authority`, `site_metrics.spam_score`, `site_metrics.root_domains_to_root_domain`, `site_metrics.external_pages_to_root_domain`
...
- DA is a 1-100 log-scaled PREDICTION of ranking ability, not a Google metric — never present it as one
- Use moz_site_metrics_multiple when profiling more than one site: same data, one billed call
- Spam Score above ~30 flags a risky link source
