...
The most-linked-to pages on the entire web by Moz's index — a reference distribution for judging whether a target's authority is actually notable.
...
```json
{
  ...
  "tool_name": "moz_v2_global_top_pages",
  "params": {}
}
```
...
- QUOTA TRAP: this endpoint has no required params, so an empty body still returns and BILLS 25 rows — always pass limit
- Reference data, not client data — use it for calibration, never as a finding on its own
