...
Age and size of the current Moz link index.
...
```json
{
  ...
  "tool_name": "moz_v2_index_metadata",
  "params": {}
}
```
...
Key fields: `last_update`, `index_size`, `next_update`
...
- Never cached — the whole point is freshness
- Gated on quota (403 insufficient-quota), so it is not free despite returning a single value
