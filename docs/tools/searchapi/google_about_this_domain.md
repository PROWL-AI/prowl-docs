...
Google About This Domain — retrieve the domain info panel that Google shows (registration date, similar sites, description).
...
```json
{
  ...
  "tool_name": "google_about_this_domain",
  "params": {
    "q": "example query"
  }
}
...
|-------|------|----------|---------|-------------|
| `q` | string | yes |  | Domain name to look up (e.g. 'example.com') |
...
```json
{
  "error": "We could not retrieve results for this search. Please try again later."
}
```
