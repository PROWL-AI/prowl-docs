...
Apple App Store product reviews via SerpAPI.
...
```json
{
  ...
  "tool_name": "serpapi_apple_reviews",
  "params": {
...
|-------|------|----------|---------|-------------|
| `product_id` | string | yes |  | Apple App Store product ID |
...
```json
{
  "error": "Missing query `product_id` parameter."
}
```
...
**Chain inputs:** `{'param': 'product_id', 'from_tool': 'serpapi_apple_app_store', 'extract': 'organic_results[].id'}`
