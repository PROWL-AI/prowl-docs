---
name: serpapi_google_ai_overview
provider: SerpAPI
...
Google AI Overview results via SerpAPI.
...
```json
{
  ...
  "tool_name": "serpapi_google_ai_overview",
  "params": {
    "q": "example query"
  }
}
...
| `page_token` | string | no |  | Optional. Fetch a specific overview directly; tokens expire ~1 minute after the search that produced them. |
...
```json
{
  ...
  "properties": {
    ...
    "page_token": {
      "type": "string",
      "description": "Optional. Fetch a specific overview directly; tokens expire ~1 minute after the search that produced them."
    }
  },
  "required": [
    "q"
