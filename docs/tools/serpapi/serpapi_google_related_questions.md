---
name: serpapi_google_related_questions
provider: SerpAPI
...
Google 'People Also Ask' questions via SerpAPI — useful for content ideation.
...
```json
{
  ...
  "tool_name": "serpapi_google_related_questions",
  "params": {
    "q": "example query"
  }
}
...
| `next_page_token` | string | no |  | Optional. Expand one question's follow-ups, using a token from a previous related-questions result. |
...
```json
{
  ...
  "properties": {
    ...
    "next_page_token": {
      "type": "string",
      "description": "Optional. Expand one question's follow-ups, using a token from a previous related-questions result."
    }
  },
  "required": [
    "q"
...
```json
{
  "error": "Missing query `next_page_token` parameter."
}
```
