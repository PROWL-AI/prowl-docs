...
Get historical ad copy and keywords for a domain.
...
```json
{
  ...
  "tool_name": "spyfu_get_ad_history",
  "params": {
    "domain": "example.com"
  }
}
...
| `results[].searchDateId` | integer |  |
| `results[].title` | string |  |
| `results[].body` | string |  |
| `results[].url` | string |  |
| `results[].adId` | integer |  |
  ...
  "results": [
    {
      "keywords": [
        ...
        "new instagram account for business",
        "instag
...
```
...
- Domain-level ad history — shows messaging evolution over time
- For keyword-level ad history, use spyfu_get_term_ad_history instead
- Combine with spyfu_get_paid_search for current + historical view
