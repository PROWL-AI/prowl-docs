---
name: spyfu_get_most_valuable_keywords
provider: SpyFu
provider_slug: spyfu
category: seo
...
Get the most valuable organic keywords for a domain, sorted by SEO clicks.
...
```json
{
  ...
  "tool_name": "spyfu_get_most_valuable_keywords",
  "params": {
    "domain": "example.com"
  }
}
...
| `results[].topRankedUrl` | string |  |
| `results[].rank` | integer |  |
| `results[].rankChange` | integer |  |
...
- SEO analysis workflow: reveals keyword strategy and highest-value terms
- Use after spyfu_get_domain_stats and spyfu_get_top_pages
- Pair with spyfu_get_ppc_keywords to compare organic vs paid keyword strategies
